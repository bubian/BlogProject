...
import com.pds.tools.business.med.env.EnvUtils;
...
public class ApiInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (EnvUtils.type(request.url().host()) < 0) {
            return chain.proceed(request);
        }
        boolean b = PreferencesManager.getBoolean(DoraemonKit.APPLICATION, PreferencesKey.MED_API_CAPTURE);
        if (!b || ApiCaptureManager.instance().size()) {
            return chain.proceed(request);
        }
        ...
        ApiCaptureManager.instance().addData(entity);
        return response;
    }
