...
public final class XdLog {
    ...
    private static String getFormatMsg(String msg, Object[] args) {
        ...
        if (TextUtils.isEmpty(result.trim()) || !result
                .contains(XdSystemUtil.objectToString(args[args.length - 1]))) {
            StringBuilder builder = new StringBuilder(msg);
            for (Object arg : args) {
                builder.append(" ").append(XdSystemUtil.objectToString(arg));
            }
    ...
    private static void logObject(LogType type, String tag, Object object) {
        ...
        if (object != null) {
            ...
            if (object instanceof Throwable) {
                ...
                int dim = XdArrayUtil.getArrayDimension(object);
                switch (dim) {
                    case 1:
                        Pair pair = XdArrayUtil.arrayToString(object);
                        msg = simpleName.replace("[]", "[" + pair.first + "] {\n");
                        msg += pair.second + "\n";
                        break;
                    case 2:
                        Pair pair1 = XdArrayUtil.arrayToObject(object);
                ...
                if (!collection.isEmpty()) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        msg += String.format(itemString, flag, XdSystemUtil.objectToString(item),
                ...
                for (Object key : keys) {
                    ...
                    msg += String.format(itemString, XdSystemUtil.objectToString(key),
                                         XdSystemUtil.objectToString(value));
                }
                logString(type, tag, msg + "}");
            } else {
                logString(type, tag, XdSystemUtil.objectToString(object));
            }
        } else {
            logString(type, tag, XdSystemUtil.objectToString(object));
        }
