...
public class XdMemoryOutputStream extends OutputStream {
	int _writePos = 0;
	int _maxSize = 0;
	byte[] _dataBuf;
	volatile int _inStreamNum = 0;

	public XdMemoryOutputStream() {
		this(0);
	}

	public XdMemoryOutputStream(int initialCapacity) {
	...
	public InputStream toInputStream(int offset, int len) {
		...
		InputStream inputStream = new InputStream() {
			...
			byte[] _dataBuf = XdMemoryOutputStream.this._dataBuf;
