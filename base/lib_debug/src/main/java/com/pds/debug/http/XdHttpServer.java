...
import com.pds.debug.work.XdWorker;
...
public class XdHttpServer {
	private static final String TAG = XdHttpServer.class.getSimpleName();
	...
	public XdHttpServer(String addr, int port) {
		_initServer(addr, port);
	}

	public XdHttpServer(int port) {
	...
	private void _initServer(String addr, int port) {
		_localAddr = addr;
		_localPort = port;
		_workerGroup = new ThreadGroup("Debug HTTP Server");
		ThreadFactory threadFactory = r -> new Thread(_workerGroup, r, "Reactor");
		...
		synchronized (this) {
			if (_selector == null) {
				try {
					...
					InetSocketAddress localAddress = TextUtils.isEmpty(_localAddr) ?
	...
	public static class XulHttpServerRequest extends XdHttpRequest {
	...
	public static class XulHttpServerResponse extends XdHttpResponse {
		...
		public XdHttpResponse cleanBody() {
	...
	public static class XulHttpServerHandler {
		private final XdHttpServer _server;
		...
		public XulHttpServerHandler(XdHttpServer server, SocketChannel socketChannel) {
			_server = server;
		...
		class HttpRequestBuilder {
			private XdMemoryOutputStream _readBuffer;
			...
			public HttpRequestBuilder() {
				_readBuffer = XdWorker.obtainDownloadBuffer(4096);
			}
				...
				while (_parseState != 2 && (line = readLine()) != null) {
					if (_parseState == 0) {
						...
						if (headerKey.equalsIgnoreCase("content-length")) {
							_requestBodySize = XdUtils.tryParseInt(headerValue);
						} else if (headerKey.equalsIgnoreCase("host")) {
							int hostPortPos = headerValue.indexOf(":");
							if (hostPortPos > 0) {
								_request.port = XdUtils.tryParseInt(headerValue.substring(hostPortPos + 1));
