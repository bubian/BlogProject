...
public class SuitLines extends View {
    ...
    private void calcAreas() {
         ...
         * 垂直方向刻度标题的最大宽带
         */
        float textWidth = Math.max(xyPaint.measureText(String.valueOf(minAndMaxOfY[0])), xyPaint.measureText(String.valueOf(minAndMaxOfY[1])));
        ...
        yArea = new RectF(validArea.left, validArea.top, validArea.left + maxWidth + basePadding,
    ...
    private void calcUnitXY() {
        ...
        for (int i = 0; i < datas.get(0).size(); i++) {
            for (int j = 0; j < datas.size(); j++) {
                ...
                float scale = new BigDecimal("1")
                        ////减法
                        .subtract((new BigDecimal(Float.toString(curValue))
                        .subtract(new BigDecimal(Float.toString(minAndMaxOfY[0]))))
                        //除法 ROUND_DOWN    //向零方向舍入
