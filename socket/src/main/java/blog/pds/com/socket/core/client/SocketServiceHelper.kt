...
import blog.pds.com.socket.core.common.SAction
...
class SocketServiceHelper{
    ...
    companion object {
        fun connect(context: Context,startSocketId: Long, ip: String, port: Int) {
            val intent = createImServiceIntent(context,SAction.OP_TYPE_CONNECT)
        ...
        fun disConnect(context: Context) {
            val intent = createImServiceIntent(context,SAction.OP_TYPE_DISCONNECT)
            context.startService(intent)
        }

        fun reConnect(context: Context) {
            val intent = createImServiceIntent(context,SAction.OP_TYPE_RECONNECT)
            context.startService(intent)
        }

        fun sendMessage(context: Context,msg: ByteArray) {
            val intent = createImServiceIntent(context,SAction.OP_TYPE_SEND)
