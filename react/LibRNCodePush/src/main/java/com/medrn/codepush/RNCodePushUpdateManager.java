...
import com.medrn.codepush.dialog.CodePushProgressDialog;
import com.medrn.codepush.dialog.LoadingDialogFragment;
import com.medrn.codepush.dialog.RestartRNDialog;
import com.medrn.codepush.entity.MetaInfoEntity;
import com.medrn.codepush.util.DownLoadUtil;
import com.medrn.codepush.util.FileUtils;
import com.medrn.codepush.util.RNCodePushUpdateUtils;
...
public class RNCodePushUpdateManager {
        ...
        @SuppressLint("StaticFieldLeak") AsyncTask<Void, Integer, Boolean> asyncTask = new AsyncTask<Void, Integer, Boolean>() {
            ...
            protected Boolean doInBackground(Void... voids) {
                ...
                try {
                    ...
                    String version = currentMetaInfo.getBaseVersion();
                    isCompleted = DownLoadUtil.download(mServerUrl, version, RNCodePushConstants.STATUS_FILE_NAME,
    ...
    private void compareUpdate(final Context context, boolean auto, CallBack<Boolean> checkUpdateCallBack,
        ...
        if (auto) {
            ...
            mUpdateDialog = new AlertDialog.Builder(context)
                    ...
                    .setMessage(compareMetaInfo.getUpdateContent())
    ...
    public boolean isUpdate(MetaInfoEntity compareMetaInfo, MetaInfoEntity localMetaInfo) {
        return !TextUtils.equals(compareMetaInfo.getHash(), localMetaInfo.getHash())
        ...
        mDownloadPackageAsyncTask = new AsyncTask<Void, DownloadProgress, Boolean>() {
            ...
            protected Boolean doInBackground(Void... voids) {
                ...
                try {
                    ...
                    //下载热更文件到"下载目录"并解压
                    isComplete = DownLoadUtil.download(mServerUrl, updateMetaInfo.getBaseVersion(), updateMetaInfo.getDownloadFile(),
                    ...
                    isComplete = false;
                    // ModuleCodePushManager.getsInstance().getModuleService().sendFeedback(String.format("RN热更新失败 hash: %s; buildTime: %s", updateMetaInfo.getHash(), updateMetaInfo.getBuildTime()));
